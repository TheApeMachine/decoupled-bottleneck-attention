#!/usr/bin/env python3
""" 
v27_transformer_decoupled_bottleneck_instrumented.py
...
"Survive scale" upgrades in this v27 variant:
...
def _run_single(args: argparse.Namespace, device: torch.device) -> None:
    ...
    def get_batch_any(view: TokenView, batch_size: int, block_size: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ix = torch.randint(0, max_start, (batch_size,), device="cpu")
        if isinstance(view.data, torch.Tensor):
            # gather slices efficiently from torch CPU tensor
            x = torch.stack([view.data[view.start + int(i): view.start + int(i) + block_size] for i in ix], dim=0)
            y = torch.stack([view.data[view.start + int(i) + 1: view.start + int(i) + block_size + 1] for i in ix], dim=0)
            ...
            for i in ix.tolist():
                i0 = view.start + int(i)
                xs.append(_np.asarray(view.data[i0:i0 + block_size], dtype=_np.int64))
                ys.append(_np.asarray(view.data[i0 + 1:i0 + block_size + 1], dtype=_np.int64))
            x = torch.from_numpy(_np.stack(xs, axis=0)).to(device)
            y = torch.from_numpy(_np.stack(ys, axis=0)).to(device)
            return x, y

    @torch.no_grad()
    def estimate_loss_any() -> Tuple[float, float]:
        model.eval()
        def _eval(view: TokenView) -> float:
            losses = []
            for _ in range(int(args.eval_iters)):
                xb, yb = get_batch_any(view, int(args.batch_size), int(cfg.block_size), device)
                logits, _ = model(xb)
    ...
    # Parse analysis layer/head lists
    ...
    layers = _parse_csv_int_list(args.analysis_layers) if args.analysis_layers else [0, -1]
    heads = _parse_csv_int_list(args.analysis_heads) if args.analysis_heads else [0]
    ...
    try:
        ...
        for step in range(args.steps + 1):
            ...
            if step % args.eval_every == 0:
                tr, va = estimate_loss_any()
                ppl = math.exp(va) if va < 20 else float("inf")
                msg = f"== eval step {step} | train {tr:.4f} | val {va:.4f} | val_ppl {ppl:.2f} | {(time.time()-t_eval0):.1f}s"
                print(msg)
                if logger is not None:
                    logger.log({"type": "eval", "step": step, "train_loss": float(tr), "val_loss": float(va), "val_ppl": float(ppl)})
                if va < best_val:
                    best_val = va
            ...
            # Deep analysis (attention matrices + SVD)
            # Run for basic/medium/full, but skip heavy saving/SVD unless full
            do_analysis = (args.instrument != "off") and (args.analysis_every > 0) and (step % args.analysis_every == 0)
            if do_analysis and logger is not None:
                try:
                    is_full = (args.instrument == "full")
                    x_an, _ = get_batch_any(val_view, 1, cfg.block_size, device)
                    stats, tensors = analyze_attention(
                        model, x_an,
                        layers=layers,
                        heads=heads,
                        max_tokens=int(args.analysis_max_tokens),
                        topk=int(args.analysis_topk),
                        local_window=int(args.analysis_local_window),
                        save_mats=is_full,
                        save_scores=bool(args.analysis_save_scores) and is_full,
                        compute_svd=is_full,
                    )
                    ev = {"type": "analysis", "step": step, **{k: float(v) for k, v in stats.items()}}
                    logger.log(ev)
                    if tensors and is_full:
                        logger.h5_write_step(step, group="analysis", tensors=tensors, attrs={"step": int(step)})
                except Exception as e:
                    logger.log({"type": "analysis", "step": step, "error": str(e)})

            if step == args.steps:
                break

            # Train step
            xb, yb = get_batch_any(train_view, int(args.batch_size), int(cfg.block_size), device)
            t0 = time.time()
            ...
            if scaler.is_enabled():
                ...
                if args.grad_clip > 0:
                    ...
                    grad_stats["grad_clip_total_norm"] = float(total_norm)
                scaler.step(opt)
                scaler.update()
            else:
                loss.backward()
            ...
            if step % args.log_every == 0:
                ...
                if logger is not None:
                    ev = {"type": "train", "step": step, "loss": float(loss.item()), "ppl": float(ppl), "tok_s": float(tok_s)}
                    ev.update({k: float(v) for k, v in grad_stats.items()})
                    logger.log(ev)
                tok_count = 0
                dt_acc = 0.0
        ...
        if logger is not None:
            logger.finalize(best_val=float(best_val), last_step=int(args.steps))
    finally:
        if logger is not None:
            logger.close()
