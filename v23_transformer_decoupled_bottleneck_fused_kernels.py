#!/usr/bin/env python3
""" 
v23_transformer_decoupled_bottleneck_fused_kernels.py
...
"Survive scale" upgrades in this v23 variant:

  A) Streaming ("online softmax") decode that dequantizes only small sequence blocks.
  B) Optional fused GPU kernels (Triton, if installed) that fuse dequant + attention update
     per block to avoid materializing dequantized K/V in PyTorch.
...
class GPT(nn.Module):
    ...
    def generate(
        ...
        *,
        ...
        kv_fused: str = "auto",  # {none, auto, triton}
        ...
        if kv_fused not in ("none", "auto", "triton"):
            raise ValueError("kv_fused must be one of: none, auto, triton")
...
def main() -> None:
    ...
    ap.add_argument("--kv-fused", type=str, default="auto", choices=["none", "auto", "triton"],
                    help="Use fused decode kernels when available. 'auto' enables Triton kernels if Triton is installed and CUDA is available.")
